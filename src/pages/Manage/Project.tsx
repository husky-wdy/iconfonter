...
import { createCdnFiles, fetchFile, fetchIconItem, fetchProjectInfo } from '../../api'
  ...
  generateSVGZip = () => {
    this.runGenerator(async () => {
      ...
      for (let i = 0; i < icons.length; i++) {
        const { show_svg } = await fetchIconItem({ id: icons[i].id, pid: id })
        console.log(show_svg)
        const fileName = `${svgZip.withPrefix ? `${project.prefix}-` : ''}${icons[i].font_class}.svg`
        const fileContent = dedent`${show_svg}`
